...
final class Store: ObservableObject {
    @Published private(set) var items: [SyllabankItem] = []
    ...
    init() {
        let dir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Syllabank", isDirectory: true)
    ...
    func add(title: String, note: String = "", value: Double = 0, date: Date = Date()) -> Bool {
        ...
        let item = SyllabankItem(title: title, note: note, value: value, date: date)
        items.insert(item, at: 0)
        save()
        return true
    }

    func update(_ item: SyllabankItem) {
    ...
    func delete(_ item: SyllabankItem) {
        items.removeAll { $0.id == item.id }
        save()
    }

    func toggleDone(_ item: SyllabankItem) {
        guard let idx = items.firstIndex(where: { $0.id == item.id }) else { return }
    ...
    private func load() {
        if let data = try? Data(contentsOf: fileURL),
           let decoded = try? JSONDecoder().decode([SyllabankItem].self, from: data) {
            items = decoded
        } else {
    ...
    static var seedData: [SyllabankItem] {
        let now = Date()
